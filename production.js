/*
 * MONGODB VERSION FOR CLUSTERING
 * THIS IS A TEST
 */
...
db.once('open', function callback () {
      ...
      eztv.getAllEpisodes(show, function(err, data) {
          ...
          if(!dateBased) {
            ...
            function(err, res) {
             ...
             TVShow.find({imdb_id: show.imdb}, function(err, show) {
                 if(err) return callback(err, null);
                 if(show.episodes != thisEpisodes) {
                     TVShow.update({ imdb_id: imdb },
            ...
            trakt.request('show', 'summary', {title: imdb, extended: "full"}, function(err, show) {
              if(!show) return callback(null, show);
               ...
               // Only change "lastUpdated" date if there are new episodes
               TVShow.find({imdb_id: imdb}, function(err, show) {
                   if(err)  return callback(err, null);
                   if(show.episodes != thisEpisodes) {
                       TVShow.update({ imdb_id: imdb },
                           { $set: { episodes: thisEpisodes, last_updated: +new Date(), num_seasons: numSeasons}},
  ...
  function extractTrakt(show, callback) {
      ...
      var slug = show.slug;
        ...
        TVShow.find({slug: show.slug}, function(err, doc){
  ...
  server.get('/shows/update/:since', function(req, res) {
      ...
      TVShow.count({last_updated : {$gt: parseInt(since)},num_seasons: { $gt: 0 }}, function (err, count) {
  ...
  server.get('/shows/update/:since/:page', function(req, res) {
      ...
      TVShow.find({last_updated : {$gt: parseInt(since)},num_seasons: { $gt: 0 }}).sort({ title: -1 }).skip(offset).limit(byPage).exec(function (err, docs) {
        res.json(202, docs);
      });
  ...
  server.get('/shows/last_updated', function(req, res) { 
      TVShow.find({num_seasons: { $gt: 0 }}).sort({ last_updated: -1 }).limit(byPage).exec(function (err, docs) {
        res.json(202, docs);
      });
  });
  ...
  server.get('/shows/last_updated/:page', function(req, res) {
      var page = req.params.page-1;   
      var offset = page*byPage;
      TVShow.find({num_seasons: { $gt: 0 }}).sort({ last_updated: -1 }).skip(offset).limit(byPage).exec(function (err, docs) {
        res.json(202, docs);
      });
  });
  ...
  server.get('/shows/search/:search', function(req, res) {
      ...
      TVShow.find({title: keywords,num_seasons: { $gt: 0 }}).sort({ title: -1 }).limit(byPage).exec(function (err, docs) {
        res.json(202, docs);
      });
  });
  ...
  server.get('/shows/search/:search/:page', function(req, res) {
      ...
      var offset = page*byPage;    
      var keywords = new RegExp(req.params.search.toLowerCase(),"gi");
      TVShow.find({title: keywords,num_seasons: { $gt: 0 }}).sort({ title: -1 }).skip(offset).limit(byPage).exec(function (err, docs) {
        res.json(202, docs);
      });
  });

  server.get('/show/:id', function(req, res) {
      TVShow.find({imdb_id: req.params.id}).limit(1).exec(function (err, docs) {
  ...
  server.get('/shows/updated/:since', function(req, res) {
      var since = req.params.since;
      TVShow.find({last_updated : {$gt: parseInt(since)},num_seasons: { $gt: 0 }}, function(err, docs) {
          res.json(202, docs);
      })
  });
