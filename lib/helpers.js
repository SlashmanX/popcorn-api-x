...
var missing = new (winston.Logger)({
  transports: [
    new (winston.transports.File)({ filename: 'missing.log' })
  ]
});
...
var helpers = {
  ...
  addEpisodeToDb: function(episode, callback) {
	...
	logger.info('Searching Show: '+ episode.title);
    	...
    	if (!err && data && data[0]) {
    		...
    		Show.findOne({ imdb_id: show.imdb_id }, function(err, doc){
    			...
    			if(episodes[e]) { // exists add torrent to existing info
    				...
    				trakt.request('show', 'episode/summary', 
    					{title: show.tvdb_id, season: episode.season, episode: episode.episode}, 
    					function(err, data) {
