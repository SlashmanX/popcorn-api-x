...
var helpers = {
  ...
  update: function(callback) {
  	kickass.update('', function(err, updated) {
  		console.log('done');
  		async.mapLimit(updated, 2, helpers.addEpisodeToDb, function(err, results){
  ...
  addEpisodeToDb: function(episode, callback) {
  	...
  	trakt.request('search', 'shows', {query: episode.title, limit: 1}, function(err, data) {
		...
		if (!err && data && data[0]) {
			var show = data[0];
			console.log('Searching: '+ episode.title + ' / Found: '+ show.title)
			Show.findOne({ imdb_id: show.imdb_id }, function(err, doc){
				if(err || !doc) {
					console.log('Show not in database: '+ episode.title);
				...
				if(episodes[e]) { // exists add torrent to existing info
					if(!episodes[e].torrents[episode.quality] || episode.filename.indexOf("REPACK") !== -1) {
						...
						console.log('Added torrent to existing episode');
					}
				}
				...
				Show.update({imdb_id: show.imdb_id}, {$set: {episodes: episodes}}, function(err, doc){
					return callback(null, episode);
				});
		    });
		}
